...
- 품목·장비 등록, 검색, **품목 목록(검색 없이 브라우즈, 유형·재고부족 필터)**, **기자재 현황(상태·실·연체·사업예산 필터)**, **실험실습재료 현황(소모품·부품 재고·최소재고·부족·실·사업예산)**, 실별 조회, 장비 대여·반납·이동.
- 사진 업로드, 장비 고장 신고, QR 스캔 및 라벨 인쇄.
- 소모품·부품 분출(사용 출고): 품목 상세에서 위치별 수량·실 선택·사용 사유, 선택 수업/메모.
  - 관리자·담당교사·일반교사(`canLoan`)가 분출 가능. POST+CSRF.
  - 소수 수량 지원, 재고 초과·0 이하·비정상 수량 및 다른 품목의 재고 지정 차단.
  - 조건부 UPDATE로 잔량 검사와 차감 처리. 이력 저장까지 같은 트랜잭션으로 묶음.
  - 실(`kind=room`)은 `activity_logs.meta_json.room_id`에 남김. 품목 상세·재료 보드에서 실별 이력 필터.
  - 사용 출고는 반환할 대여가 아니므로 `loans` 대신 `activity_logs`에 기록. 출고 취소 경로는 그대로.
...
- 연한·노후 기자재 보드(#52): `assets/aging`. 로그인(기자재 현황과 동일). #39 도입일+내용연한으로 만료일 계산. 기본 목록은 임박(오늘~365일) ∪ 초과(만료일 이전). GET 필터 `life=imminent|exceeded`(임박/초과 별칭 허용). 더보기·기자재 현황에서 진입. 조달청 제안은 장비 상세 기존 UI. 보드는 읽기만. 보정·파기는 #53.
- 실사 차이 보정·파기(#53): 종료된 실사 미확인 라인을 `Inventory::adjust`(사유·승인자, POST+CSRF, `canInventory`/`canWrite`). 품목 로트 수량 맞춤, 장비는 `lost`. 파기는 `Asset::retire`로 `retired`(사유·일자·선택 증빙, 되돌릴 수 없음, manager+). 이력은 `activity_logs`. 수리비(#54) 없음.
...
- `php tests/stock_issue_room.php`: 분출 실/수업메모, 이력 실 필터, 출고 취소 유지, #48 로트/부족 회귀.
...
- `php tests/asset_life_board.php`: 임박/초과 판정, 목록·필터, 잘못된 필터 무시, 더보기 진입, #53/#54 미포함.
...
- `php tests/inventory_adjust.php`: 종료 실사 미확인 보정(사유·승인자), 품목 수량/`lost`, 권한·이중 보정·롤백.
- `php tests/asset_retire.php`: 파기 사유·일자·증빙, 대여중 거부, 이중 파기 실패 폐쇄, 이력.
...
4j. ~~실사 차이 보정·파기.~~ issue #53. 수리비(#54)는 별도.

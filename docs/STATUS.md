...
- 소모품·부품 **분출**(사용 출고): 품목 상세에서 위치별 수량, **실(필수)**, 사용 사유, 수업 메모(선택).
  - 관리자·담당교사·일반교사가 분출 가능(`canLoan`).
  ...
  - 분출 실·수업 메모는 `activity_logs.meta_json`에 기록. 이력은 실 GET 필터(`room`)로 걸러봄.
  - 분출은 반환할 대여가 아니므로 `loans` 대신 `activity_logs`에 기록.
...
- `php tests/stock_issue.php`: 분출 실 필수·수업 메모 선택, 이력 실 필터, canLoan, 폼/권한. #50 보드 CTA 없음.

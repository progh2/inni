...
/** @var string $scanAction */
/** @var string $scanSubmitLabel */
$scanSubmitLabel = $scanSubmitLabel ?? '열기';
$scanHintText = $scanHintText ?? '카메라 권한이 필요합니다. HTTPS 또는 localhost에서 동작합니다.';
?>
<div class="card scan-stage" style="margin-top:1rem">
  <div id="qr-reader" aria-label="카메라 미리보기"></div>
  <p class="muted" id="scan-hint" style="margin-top:0.5rem"><?= Support::e($scanHintText) ?></p>
</div>

<form id="scan-manual-form" class="card" method="post" action="<?= Support::e($scanAction) ?>">
  <?= Csrf::field() ?>
  <div class="field">
  ...
  <button class="btn btn-primary btn-block" type="submit"><?= Support::e($scanSubmitLabel) ?></button>
</form>

<script src="https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js"></script>
<script>
(() => {
  const formAction = <?= json_encode($scanAction, JSON_UNESCAPED_SLASHES) ?>;

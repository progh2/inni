...
- 도입일·내용연한: 장비에 도입일(`purchase_date`)과 내용연한(년). 둘 다 있으면 만료 예정일 표시. 빈 값 허용. 조달청 자동 제안은 없음
- 설정·사용자 승인

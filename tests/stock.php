...
Stock::cancelIssue($pdo, $owner, 'consumable', $issueLog['id'], '수량 오입력');
...
check($cancel && (float) $cancel['quantity'] === 1.5 && $cancel['reason'] === '수량 오입력' && $cancel['actor_id'] === 'owner', 'Cancel history row missing');
...
rejectCancel($pdo, $owner, 'consumable', $issueLog['id']);
rejectCancel($pdo, $owner, 'consumable', $issueLog['id'], '   ');
rejectCancel($pdo, $owner, 'part', $issueLog['id']);
rejectCancel($pdo, $owner, 'consumable', 'missing');
rejectCancel($pdo, $owner, 'consumable', $restockLog['id']);
rejectCancel($pdo, $actor, 'consumable', $issueLog['id']);
rejectCancel($pdo, array_replace($owner, ['role' => 'student']), 'consumable', $issueLog['id']);
rejectCancel($pdo, array_replace($owner, ['status' => 'disabled']), 'consumable', $issueLog['id']);

...
function reject(PDO $pdo, array $actor, mixed $quantity, string $purpose = '수업', string $item = 'consumable', string $lot = 'lot-consumable', string $roomId = 'room'): void
{
    $before = snapshot($pdo);
    try {
        Stock::issue($pdo, $actor, $item, $lot, $quantity, $purpose, $roomId);
...
Stock::issue($pdo, $actor, 'consumable', 'lot-consumable', '2.5', '실습', 'room');
...
check($log['actor_id'] === 'teacher' && $log['entity_id'] === 'consumable' && $meta['quantity'] === 2.5 && $meta['purpose'] === '실습' && $meta['room_id'] === 'room', 'Audit metadata missing');
...
try {
    Stock::issue($pdo, $actor, 'part', 'lot-part', '1', '롤백 확인', 'room');
...
Stock::issue($pdo, $actor, 'consumable', 'lot-consumable', '1.5', '오입력', 'room');
...
Stock::issue($pdo, $actor, 'part', 'lot-part', '1', '취소 롤백', 'room');

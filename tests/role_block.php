...
check(str_contains($deskCtl, 'Auth::canLoan($user)') && str_contains($deskCtl, 'function index'), 'desk GET requires canLoan');
check(str_contains($deskCtl, 'Auth::canLoan($user)') && str_contains($deskCtl, 'function resolve'), 'desk resolve requires canLoan');
check(str_contains($deskCtl, 'Csrf::requirePost()'), 'desk resolve is POST+CSRF');

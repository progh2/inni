...
function seedMine(PDO $pdo): void
{
...
check(Loan::listMine($pdo, '') === [] && Loan::listMine($pdo, 'teacher') === [], 'empty inbox before any checkout');
check(!Loan::isOverdue(['status' => 'active']) && Loan::isOverdue(['status' => 'overdue']), 'isOverdue follows loan.status');
check(!Loan::isOverdue(['status' => 'returned']) && !Loan::isOverdue([]), 'returned or missing status is not overdue');
...
$mine = Loan::listMine($pdo, 'teacher');
...
check(!Loan::isOverdue($mine[0]), 'on-time own loan is not highlighted overdue');
...
check(Loan::isOverdue($mine[0]), 'overdue own loan is highlighted');
...
check(Loan::isOverdue($mine[0]) && !Loan::isOverdue($mine[1]), 'inbox order is overdue then due date');

$other = Loan::listMine($pdo, 'teacher-b');
check(ids($other) === [$otherOpen], 'other teacher inbox stays isolated');
check(Loan::listMine($pdo, 'student') === [], 'student with no borrower_user_id loans sees empty inbox');

Loan::checkin($pdo, $teacher, $mineSecond);
check(ids(Loan::listMine($pdo, 'teacher')) === [$mineActive], 'returned loan leaves the open inbox');
check(ids(Loan::listMine($pdo, 'teacher-b')) === [$otherOpen], 'other teacher inbox is unchanged after my return');
...
$loanSrc = (string) file_get_contents($root . '/app/Loan.php');

check(str_contains($router, "'loans/mine' => [LoanController::class, 'mine']"), 'loans/mine GET route is registered');
check(str_contains($ctl, 'function mine') && str_contains($ctl, 'Loan::listMine') && str_contains($ctl, 'Alert::refreshOverdue'), 'mine refreshes overdue and uses listMine');
...
check(str_contains($mineTpl, 'Loan::isOverdue') && str_contains($mineTpl, 'is-overdue'), 'inbox marks overdue rows via isOverdue');
...
check(str_contains($homeCtl, 'Loan::listMine') && str_contains($homeCtl, 'Auth::canLoan'), 'home loads personal inbox for canLoan roles');
...
check(str_contains($loanSrc, 'BEGIN IMMEDIATE') && substr_count($loanSrc, 'rowCount() !== 1') >= 3, 'loan writes still use BEGIN IMMEDIATE and rowCount');
check(!str_contains($ctl, 'reports') && !str_contains($mineTpl, '수리'), 'inbox does not add repair (#46)');
check(!str_contains($ctl, 'desk') && !str_contains($mineTpl, '대여 데스크'), 'inbox does not add loan desk (#47)');

echo "PASS: {$checks} loan-mine checks\n";
